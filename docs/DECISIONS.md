...
| 76 | **The cutover is reversible and was reversed.** Executed and rolled back on 2026-09-03: the app is not member-ready (placeholder copy, 49 of 103 published lessons empty — `docs/GO-LIVE-BLOCKERS.md`). `app.familyinvestingclub.com` serves `fta-dashboard` again, FTA owns the crons again, and `NEXT_PUBLIC_SITE_URL` points back at FIC's own Vercel host. Infrastructure readiness and product readiness are separate gates; the first is proven, the second is not. |
