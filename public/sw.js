/* FIC service worker (ported from fta-dashboard-v3 public/sw.js) — Web Push + notification click + self-healing subscriptions.
 *
 ...
 */
const SW_VERSION = "2026-08-28-1"; // FIC cutover: same VAPID key, FIC branding
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "Family Investing Club", body: event.data && event.data.text() };
  }

  const title = data.title || "Family Investing Club";
  const options = {
    body: data.body || "",
