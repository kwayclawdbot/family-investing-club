...
import type { ClubChatMessage, DecisionRecord, OfficialPicks as Picks } from "@/lib/live/club";
...
type Props = { club: Club; overview: ClubOverview; portfolio: PortfolioTab; members: MemberCard[]; exposure: VerifiedExposure; initialTab?: WorkspaceTab; forceNew?: boolean; chat?: ClubChatMessage[] | null; circles?: CircleView[] | null; official?: Picks | null; record?: DecisionRecord | null };
...
export function ClubWorkspace({ club, overview, portfolio, members, initialTab = "chat", forceNew, chat, circles, official, record }: Props) {
    ...
    <>
      <div className="mt-[14px] flex items-center gap-[10px]">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[9.5px] font-extrabold text-ink-3">{privacy === "private" ? "🔒 Private" : "🌍 Public"} · {overview.members} {overview.members === 1 ? "member" : "members"}</div>
        </div>
...
/** Kai's club line, counted rather than written: what the club holds, is deciding and is researching. */
function kaiSummary(o: ClubOverview, official?: Picks | null): string | undefined {
  const bits = [
    official?.count ? `${official.count} ${official.count === 1 ? "holding" : "holdings"} voted in` : null,
    o.activeDecision ? `1 open vote (${o.activeDecision.voted}/${o.activeDecision.eligible} in, ${o.activeDecision.hoursLeft}h left)` : null,
    o.research.length ? `${o.research.length} research ${o.research.length === 1 ? "note" : "notes"} in progress` : null,
  ].filter(Boolean);
