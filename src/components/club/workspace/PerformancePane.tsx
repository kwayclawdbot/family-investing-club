...
export function PerformancePane({ o, p }: { o: ClubOverview; p: PortfolioTab }) {
  ...
  const vsBench = o.benchmarkPct === null || o.benchmarkPct === undefined ? null : +(o.ytdPct - o.benchmarkPct).toFixed(1);
  ...
  return (
    <>
      <div className="mt-[13px]">
        <span className={cx("text-[34px] font-black", o.ytdPct >= 0 ? "text-[#3A8C4A]" : "text-red")}>{pctText(o.ytdPct)}</span>
        <span className="ml-2 text-[13px] font-extrabold text-ink-3">YTD · ${o.value.toLocaleString()} practice stake</span>
      </div>
      {s ? <PerfChart club={s.club} benchmark={s.benchmark} markers={s.markers} height={92} className="mt-1" />
         : <p className="mt-2 mb-1 rounded-[12px] border border-line bg-card px-3 py-5 text-center text-[11.5px] font-bold text-ink-3">No price history for these holdings yet.</p>}
      <div className="flex gap-[5px] mt-1 items-center">
        {(o.series.length ? o.ranges : []).map((x) => <button key={x} onClick={() => setRange(x)} className={cx("px-2 py-1 text-[10.5px] rounded-[8px]", x === range ? "bg-ink text-cream-text font-black px-[11px]" : "text-ink-4 font-extrabold")}>{x}</button>)}
      ...
      <div className="flex gap-2 mt-3">
        {[["VS S&P 500", vsBench === null ? "—" : pctText(vsBench), vsBench !== null && vsBench < 0 ? "text-red" : "text-[#3A8C4A]"],
          ["POSITIVE PICKS", o.metrics.winRatePct === null ? "—" : `${o.metrics.winRatePct}%`, "text-ink"]].map(([l, v, c]) => (
        ...
        <div className="flex-1 bg-card border border-line rounded-[13px] px-3 py-[10px] text-center"><div className="text-[9px] font-black text-ink-3">BEST PICK</div><div className="text-[16px] font-black text-ink mt-[2px]">{o.metrics.bestPick ? <>{o.metrics.bestPick.symbol} <span className="text-[10px] text-[#3A8C4A]">{pctText(o.metrics.bestPick.pct)}</span></> : "—"}</div></div>
      </div>
      ...
      <Panel className="px-[14px] py-[3px]">
        {!o.topInvestors.length && <p className="py-4 text-center text-[11.5px] font-bold text-ink-3">No picks priced yet — the board fills in as members make picks.</p>}
      ...
      <Panel className="px-[15px] py-3 flex gap-[14px] items-center">
        ...
        <div className="flex-1 flex flex-wrap gap-x-3 gap-y-[3px] text-[10.5px] font-extrabold text-[#4A4436]">
          {p.allocation.map((a) => (
            <span key={a.label} className="inline-flex items-center gap-[5px]"><span className="w-[9px] h-[9px] rounded-[3px]" style={{ background: DONUT[a.color] ?? "#E4DAC4" }} />{a.label} {a.pct}%</span>
          ))}
        </div>
      </Panel>
    </>
