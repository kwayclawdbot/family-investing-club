...
import { Check, X, TrendingUp, TrendingDown, ArrowRight } from "./icons";
import { EmptyLine, TextAction } from "./parts";
...
export default function TrendOrTrapGame() {
  ...
  // reset per round
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- animation phase machine: the effect is the transition
    setPhase("popin");
    // eslint-disable-next-line react-hooks/set-state-in-effect -- animation phase machine: the effect is the transition
    setRevealed(0);
    // eslint-disable-next-line react-hooks/set-state-in-effect -- animation phase machine: the effect is the transition
  ...
  if (g.done) {
    return (
      <GameEndScreen
        ...
        onReplay={g.replay}
        backHref="/learn/games"
        backLabel="Back to the arcade"
      />
    );
  }
