...
import { siteUrl } from "@/lib/server/shared/site-url";
import { notConfigured, notConfiguredResponse, logNotConfigured } from "@/lib/server/env";
import { parseNotificationPayload, type NotificationRow, type NotifType } from "@/lib/server/push-payload";
import {
...
// PORTED-VERBATIM from fta-dashboard-v3 src/app/api/push/dispatch/route.ts. NotificationRow / NotifType and the
// payload guard live in @/lib/server/push-payload so scripts/platform-smoke.mjs can dry-run them.
...
function vapidConfigured(): boolean {
  const ok = Boolean(process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY?.trim() && process.env.VAPID_PRIVATE_KEY?.trim());
  if (!ok) logNotConfigured(process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY?.trim() ? "VAPID_PRIVATE_KEY" : "NEXT_PUBLIC_VAPID_PUBLIC_KEY");
  return ok;
}
...
export async function POST(req: NextRequest) {
  ...
  if (!secret) return notConfigured("PUSH_DISPATCH_SECRET", 401);
  ...
  try {
    ...
    return notConfiguredResponse(e) ?? NextResponse.json({ error: "database unavailable" }, { status: 503 });
  }
