...
import { getClubTier } from "@/lib/server/shared/tier";
import { deriveRegister } from "@/lib/server/shared/register";
import { memberMode } from "@/lib/server/shared/mode";
import {
  getQuote,
  getBars,
  getCompany,
  ...
  searchTickers,
  normalizeSymbol,
} from "@/lib/server/market/polygon";
import {
...
} from "@/lib/server/kai/persona";
import { beltForXp } from "@/lib/server/shared/belts";
import { serviceClient } from "@/lib/server/membership";
import { logClubEvent } from "@/lib/server/club/track";
import type { Register } from "@/lib/server/shared/register";
import { parseAlertRequest, sanitizeRuleSpec, AlertParseError } from "@/lib/server/alerts/parse";
import { MAX_ACTIVE_RULES } from "@/lib/server/alerts/types";
...
export async function POST(req: NextRequest) {
  ...
  if (!key) {
    logNotConfigured("ANTHROPIC_API_KEY");
    return Response.json({ error: "Kai is offline right now.", not_configured: "ANTHROPIC_API_KEY" }, { status: 503 });
  }
