...
import { EXPERIENCE_HEADER, parseExperience } from "@/lib/server/experience/registry";
import { notConfigured } from "@/lib/server/env";
...
export async function GET(req: NextRequest) {
  ...
  // ADAPTED for FIC: there is no embedded /checkout/club page in this app (yet), so the
  // guest entry always creates a Stripe-HOSTED session — FTA's ?fallback=1 path — and
  // 302s to checkout.stripe.com. Session config (price, metadata.kind=club_membership,
  // door, src, bump) is identical, so the webhook + provisioning are untouched.
  const sk = process.env.STRIPE_SECRET_KEY?.trim();
  if (!sk) {
    notConfigured("STRIPE_SECRET_KEY");
    return NextResponse.redirect(CLUB_CANCEL_URL, 302);
  }
...
export async function POST(req: NextRequest) {
  const sk = process.env.STRIPE_SECRET_KEY?.trim();
  if (!sk) return notConfigured("STRIPE_SECRET_KEY");
