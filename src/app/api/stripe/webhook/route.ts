...
import { parseExperience } from "@/lib/server/experience/registry";
...
// PORTED-VERBATIM from fta-dashboard-v3 src/app/api/stripe/webhook/route.ts — signature check moved to
// @/lib/server/stripe-signature (same HMAC, shared with /api/shop/webhook, unit-tested by scripts/platform-smoke.mjs).
...
export async function POST(req: NextRequest) {
  ...
  if (!secret) return notConfigured("STRIPE_WEBHOOK_SECRET");
  const payload = await req.text();
  if (!verifyStripeSignature(payload, req.headers.get("stripe-signature"), secret))
    return NextResponse.json({ error: "bad signature" }, { status: 400 });
  try {
    return await handleEvent(JSON.parse(payload));
  } catch (e) {
