...
export default async function ClubPage(props: PageProps<"/club">) {
  ...
  const [sp, club, overview, portfolio, members, exposure, chat] = await Promise.all([props.searchParams, getClub(), getClubOverview(), getPortfolioTab(), getMemberCards(), getVerifiedExposure(), getClubChat()]);
  return <ClubWorkspace club={club} overview={overview} portfolio={portfolio} members={members} exposure={exposure} initialTab={asTab(sp.tab)} forceNew={sp.state === "new"} chat={chat} />;
}
