...
import { getClubChat, getDecisionRecord, getOfficialPicks } from "@/lib/live/club";
...
export default async function ClubPage(props: PageProps<"/club">) {
  ...
  const [sp, club, overview, portfolio, members, exposure, chat, circles, official, record] = await Promise.all([props.searchParams, getClub(), getClubOverview(), getPortfolioTab(), getMemberCards(), getVerifiedExposure(), getClubChat(), getCircles(), getOfficialPicks(), getDecisionRecord()]);
  return <ClubWorkspace club={club} overview={overview} portfolio={portfolio} members={members} exposure={exposure} initialTab={asTab(sp.tab)} forceNew={sp.state === "new"} chat={chat} circles={circles} official={official} record={record} />;
}
