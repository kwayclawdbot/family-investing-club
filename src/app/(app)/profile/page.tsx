...
export default async function ProfilePage() {
  ...
  const pct = next ? Math.round((xp / next.minXp) * 100) : 100;
  ...
  return (
    <div className="pt-[14px] pb-6">
      <div className="flex flex-col items-center mt-1">
        ...
        <div className="mt-1 text-[10.5px] font-extrabold text-ink-3">{next ? `${toNext.toLocaleString()} XP to ${next.label} · ${pct}% there` : "Black Belt · apex"}</div>
      </div>
      <div className="mt-[14px] mb-[6px] text-[11px] font-black text-ink-3">MY RECORD</div>
      <div className="flex gap-2">
        {[[`${rep.pickPositivePct}%`, "PICK ACCURACY", "text-green"], [`+${myPerformance.ytdPct}%`, "PICKS YTD", "text-[#3A8C4A]"], ["12", "RESEARCH NOTES", "text-ink"]].map(([v, l, c]) => (
      ...
      <div className="mt-[10px] bg-card border border-line rounded-[15px] px-[15px] py-[3px]">
        ...
        <Link href="/kai" className={row}><span>✦ Kai history</span><span className="text-ink-4">›</span></Link>
