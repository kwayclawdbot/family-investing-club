...
/**
 * Web Push client helpers (VAPID, no vendor) — PORTED-VERBATIM from fta-dashboard-v3
 * src/lib/push.ts + src/lib/useSelfHealPush.ts. Browser-only: every call takes the browser
 * Supabase client (@/lib/supabase/client) and writes push_subscriptions under RLS as the member.
 *
...
/* ── Self-heal hook (was src/lib/useSelfHealPush.ts in FTA) ──────────────────────── */
 ...
 * (localStorage-throttled — a per-device convenience, not member data). Never prompts,
 * never renders, and only acts when Notification.permission is already 'granted'.
 * Mount it once in the app shell with the signed-in user's id and a browser client.
 */
export function useSelfHealPush(userId: string | null, supabase: SupabaseClient): void {
  useEffect(() => {
    if (!userId) return;
