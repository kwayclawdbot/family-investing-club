...
import { requireEnv, logNotConfigured } from "@/lib/server/env";

// PORTED-VERBATIM from fta-dashboard-v3 src/lib/server/marketing.ts; service client from @/lib/server/db.
...
// ADAPTED: FTA fell back to "dev-insecure-secret"; FIC fails closed (NotConfiguredError → 503 JSON).
function tokenSecret(): string {
  return requireEnv("MARKETING_TOKEN_SECRET");
}
...
export async function sendEmail(opts: {
  ...
  if (!key) {
    logNotConfigured("RESEND_API_KEY");
  ...
  try {
    ...
    const body = (await res.json().catch(() => ({}))) as { message?: string; error?: { message?: string }; id?: string };
    if (!res.ok) {
      const msg =
...
export async function sendSms(opts: {
  ...
  if (!sid || !token || !from) {
    logNotConfigured("TWILIO_ACCOUNT_SID/TWILIO_AUTH_TOKEN/TWILIO_PHONE_NUMBER");
  ...
  try {
    ...
    const body = (await res.json().catch(() => ({}))) as { message?: string; sid?: string };
    if (!res.ok) {
