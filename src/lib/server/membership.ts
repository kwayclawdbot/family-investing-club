...
// PORTED-VERBATIM from fta-dashboard-v3 src/lib/server/membership.ts; service client now comes from
// @/lib/server/db (fail-closed on a missing SUPABASE_SERVICE_ROLE_KEY).
...
export async function provisionMembership(opts: {
  ...
  if (!user) {
    ...
    const { data: linkData, error: linkErr } = await db.auth.admin.generateLink({
      type: "invite",
      email,
      options: { redirectTo: `${SITE}/auth/callback?next=/onboarding/who` },
    });
